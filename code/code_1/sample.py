...
# Runge Kutta法 ode
def Runge_Kutta_ode_sampler(score_model,
                            channel,
                            size,
                            marginal_prob_std,
                            diffusion_coeff,
    ...
    with torch.no_grad():
        for time_step in tqdm.tqdm(time_steps):
            batch_time_step1 = torch.ones(batch_size, device=device) * time_step
            batch_time_step2 = torch.ones(batch_size, device=device) * (time_step - step_size / 2)
            batch_time_step3 = torch.ones(batch_size, device=device) * (time_step - step_size)
            g1 = diffusion_coeff(batch_time_step1)
            g2 = diffusion_coeff(batch_time_step2)
            g3 = diffusion_coeff(batch_time_step3)
            k1 = -step_size * (- 0.5 * (g1 ** 2)[:, None, None, None] * score_model(x, batch_time_step1))
            k2 = -step_size * (- 0.5 * (g2 ** 2)[:, None, None, None] * score_model(x + 0.5 * k1, batch_time_step2))
            k3 = -step_size * (- 0.5 * (g2 ** 2)[:, None, None, None] * score_model(x + 0.5 * k2, batch_time_step2))
            k4 = -step_size * (- 0.5 * (g3 ** 2)[:, None, None, None] * score_model(x + k3, batch_time_step3))
            mean_x = x + (k1 + 2 * k2 + 2 * k3 + k4) / 6.
            x = mean_x
            imgs.append(mean_x.cpu().numpy())
            # Do not include any noise in the last sampling step.
    return imgs
...
if __name__ == '__main__':
    ...
    score_model.load_state_dict(
        torch.load('model/CIFAR10_score_model_150epochs.pth', map_location=lambda storage, location: storage)['state_dict'])
    ...
    samples = sampler(score_model,
                      ...
                      num_steps,
                      device=device)
    imgs = samples[-1].reshape(8, 8, 3, 32, 32)
    ...
    # sampler = Runge_Kutta_ode_sampler
    # samples = sampler(score_model,
    #                   1,
    #                   28,
    #                   marginal_prob_std_fn,
    ...
    for n_row in range(8):
        for n_col in range(8):
            ...
            f_ax.imshow(img)
            f_ax.axis("off")
    fig.savefig('CIFAR10_Euler_Maruyama_sampler.png')  # 将图像保存为文件
    ...
    # ])
    # dataset = CIFAR10('./realdata', train=True, download=True, transform=transform)
    ...
    #
    # t = torch.ones(images.shape[0], device='cuda')
    # images = images.to('cuda')
    # z = torch.randn_like(images)
    # std = marginal_prob_std_fn(t)
    # perturbed_x = images + z * std[:, None, None, None]
    #
    # sampler = pc_sampler
    # samples = sampler(score_model,
    #                   3,
    ...
    # will_show = samples[-1]
    # for i in range(16):
    #     img = will_show[i].transpose([1, 2, 0])
    #     img = np.array((img + 1.0) * 255 / 2, dtype=np.uint8)
    #     # 将数组转换为PIL图像对象
    #     img = Image.fromarray(img.astype(np.uint8))
    #     # 保存图像到文件
