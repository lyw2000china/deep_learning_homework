...
def timestep_embedding(timesteps, dim, max_period=10000):
    ...
    :param timesteps: a 1-D Tensor of N indices, one per batch element.
                      These may be fractional. shape: [N]
    :param dim: the dimension of the output.
    :param max_period: controls the minimum frequency of the embeddings.
    :return: an [N x dim] Tensor of positional embeddings. shape: [N, dim]
    """
    half = dim // 2
    freqs = torch.exp(
        -math.log(max_period) * torch.arange(start=0, end=half, dtype=torch.float32) / half
    ).to(device=timesteps.device) # shape: [dim // 2]
    args = timesteps[:, None].float() * freqs[None] # shape: [N, dim // 2]
    embedding = torch.cat([torch.cos(args), torch.sin(args)], dim=-1) # shape: [N, dim // 2 * 2]
    if dim % 2 == 1:
        embedding = torch.cat([embedding, torch.zeros_like(embedding[:, :1])], dim=-1) # shape: [N, dim]
    return embedding
...
def norm_layer(channels): # Group norm是将channels分为很多组，对每组求均值和方差，然后对每组进行归一化
...
for n_row in range(8):
    for n_col in range(8):
        ...
        f_ax.imshow(img)
        f_ax.axis("off")
fig.savefig('output3.png')  # 将图像保存为文件
...
for n_row in range(16):
    for n_col in range(16):
        ...
        f_ax.axis("off")
fig.savefig('output4.png')  # 将图像保存为文件
